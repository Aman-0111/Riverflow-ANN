...
Note that the program is largely the same as the main Neural network program with changes made to compensate for momentum.
...
class Mlp:
    def __init__(self, network_layout = (3,7,1)):
        ...
        self.biases = []
        self.weights = []
        ### Holds the difference in all the weights
        self.diff_weights = []
        ### Will store previous weights
        self.prev_weights = []
        ...
        for layout in self.weight_layout:
            ...
            ### Creates matrices of zeros the same size as the weights
            self.diff_weights.append(np.zeros(layout))
    ...
    def backProp(self,actual):
        ...
        Args:
            actual (Float): Value of predictand in row
        """
        ### Makes a deep copy of the current weights and stores them
        ...
        # Calculates delta for the layer and appends to list
        layer = []
        ...
        ### Now adding the change in weights when updating weights with standard 0.9 for alpha
        for x in range (len(self.weights[-1][0])):
            self.weights[-1][0][x] += self.lrate * self.delts[0][0] * self.layers_vals[1][x] + 0.9*self.diff_weights[-1][0][x]

        for y in range (len(self.weights[0])):
            for z in range (len(self.weights[0][y])):
                self.weights[0][y][z] += self.lrate * self.delts[1][y] * self.layers_vals[0][z] * self.layers_vals[1][x]  + 0.9*self.diff_weights[0][y][z]
        ...
        ### Assigns new value to self.diff_weights
        for i in range (len(self.weights)):
            for o in range (len(self.weights[i])):
                self.diff_weights[i][o] = np.subtract((self.weights[i][o]), (self.prev_weights[i][o]))
...
if __name__ == "__main__":
    ...
    for x in range (10000):
        ...
        # Runs validation every 50 epochs
        if not(x%50):
